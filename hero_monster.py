...
class KidsConversionGame:
    def __init__(self):
        ...
        self.base_monster_hp = 60  # Level 1
        self.monster_max_hp = self.base_monster_hp
        self.monster_hp = self.monster_max_hp
        self.hero_damage = 15
        self.monster_damage = 12
        self.monster_names = ["Slime", "Bat", "Lizard", "Goblin", "Golem", "Wyvern", "Hydra"]
        self.current_monster_name = random.choice(self.monster_names)
    ...
    def setup_ui(self):
        ...
        # ---------- Battle Frame ----------
        self.battle_frame = tk.Frame(self.root, bg='#FFFFFF', pady=10, padx=20, highlightthickness=0)
        self.battle_frame.pack(fill='x', padx=20, pady=10)

        top_battle = tk.Frame(self.battle_frame, bg='#FFFFFF')
        top_battle.pack(fill='x')

        self.level_label = tk.Label(
            top_battle,
            text=f"🏰 Dungeon Level {self.level}",
            font=('Comic Sans MS', 14, 'bold'),
            fg=self.colors['text'],
            bg='#FFFFFF'
        )
        ...
        bars = tk.Frame(self.battle_frame, bg='#FFFFFF')
        bars.pack(fill='x', pady=6)

        # Hero
        hero_box = tk.Frame(bars, bg='#FFFFFF')
        hero_box.pack(side='left', expand=True, fill='x', padx=5)
        tk.Label(hero_box, text="🧝‍♂️ Hero", font=('Comic Sans MS', 12, 'bold'), bg='#FFFFFF').pack(anchor='w')
        self.hero_hp_label = tk.Label(hero_box, text=f"HP: {self.hero_hp}/{self.hero_max_hp}", font=('Comic Sans MS', 10), bg='#FFFFFF')
        self.hero_hp_label.pack(anchor='w')
        self.hero_hp_bar = ttk.Progressbar(hero_box, orient='horizontal', length=300, mode='determinate', maximum=self.hero_max_hp)
        self.hero_hp_bar['value'] = self.hero_hp
        self.hero_hp_bar.pack(fill='x', pady=4)

        # Monster
        monster_box = tk.Frame(bars, bg='#FFFFFF')
        monster_box.pack(side='right', expand=True, fill='x', padx=5)
        self.monster_title = tk.Label(monster_box, text=f"👾 {self.current_monster_name}", font=('Comic Sans MS', 12, 'bold'), bg='#FFFFFF')
        self.monster_title.pack(anchor='e')
        self.monster_hp_label = tk.Label(monster_box, text=f"HP: {self.monster_hp}/{self.monster_max_hp}", font=('Comic Sans MS', 10), bg='#FFFFFF')
        self.monster_hp_label.pack(anchor='e')
        self.monster_hp_bar = ttk.Progressbar(monster_box, orient='horizontal', length=300, mode='determinate', maximum=self.monster_max_hp)
        self.monster_hp_bar['value'] = self.monster_hp
        self.monster_hp_bar.pack(fill='x', pady=4)

        # Question Frame
        question_frame = tk.Frame(self.root, bg='white', pady=20, padx=20)
        question_frame.pack(fill='both', expand=True, padx=20, pady=10)
        
        spaceship_label = tk.Label(question_frame, text="🚀", font=('Arial', 40), bg='white')
    ...
    def show_hint(self):
        ...
        # Try to pull the numeric value that appears in the question text
        # (keeps original quick-n-dirty parsing for familiarity)
        try:
            val = float(self.current_question.split()[3].replace(',', ''))
        except Exception:
            val = None

        if self.current_unit_to == "m":
            hint = "💡 Hint: To convert km to m, multiply by 1000!"
            if val is not None:
                hint += f"\n\nSo {self.format_number(val)} × 1000 = ?"
        else:
            hint = "💡 Hint: To convert m to km, divide by 1000!"
            if val is not None:
                hint += f"\n\nSo {self.format_number(val)} ÷ 1000 = ?"
    ...
    def show_notification(self, message):
        ...
        notification.after(1800, notification.destroy)
    ...
    def check_answer(self):
        """Check the user's answer and apply battle damage"""
        ...
        if is_correct:
            ...
            message += f"\n🗡️ You dealt {dmg} damage to the {self.current_monster_name}!"
            ...
            msg = (f"{encouragement}\n\n"
                   ...
                   f"💥 The {self.current_monster_name} hit you for {dmg} damage!")
            self.result_label.config(text=msg, fg=self.colors['error'])
            self.animate_wrong()
        ...
        # Battle resolution checks
        if self.monster_hp <= 0:
            self.on_monster_defeated()
        elif self.hero_hp <= 0:
            self.on_hero_defeated()
        else:
            # Enable next question, disable check button
    ...
    def calculate_hero_damage(self):
        """Hero damage scales mildly with level and streak."""
        return self.hero_damage + (self.level - 1) * 2 + min(self.streak, 5)

    def calculate_monster_damage(self):
        """Monster damage scales with level."""
        return self.monster_damage + (self.level - 1) * 2

    def update_battle_display(self):
        self.hero_hp_label.config(text=f"HP: {self.hero_hp}/{self.hero_max_hp}")
        self.hero_hp_bar['maximum'] = self.hero_max_hp
        self.hero_hp_bar['value'] = self.hero_hp

        self.monster_hp_label.config(text=f"HP: {self.monster_hp}/{self.monster_max_hp}")
        self.monster_hp_bar['maximum'] = self.monster_max_hp
        self.monster_hp_bar['value'] = self.monster_hp

        self.level_label.config(text=f"🏰 Dungeon Level {self.level}")
        self.monster_title.config(text=f"👾 {self.current_monster_name} (Lv {self.level})")

    def on_monster_defeated(self):
        """Advance to next level with stronger monster; small hero heal."""
        self.show_notification(f"🎊 You defeated the {self.current_monster_name}!")
        self.level += 1
        # Scale monster HP roughly exponentially/light linear
        ...
        # Slight bump to hero base damage every 3 levels
        if self.level % 3 == 0:
            self.hero_damage += 2

        self.update_battle_display()
        self.result_label.config(
            text=f"🏆 New Level {self.level}! A wild {self.current_monster_name} appears!\n"
    ...
    def on_hero_defeated(self):
        """Game over."""
        final_percentage = (self.score / self.total_questions) * 100 if self.total_questions else 0
        stats = (f"📊 Final Stats:\n"
        ...
        messagebox.showinfo("💀 Game Over", f"The hero has fallen!\n\n{stats}Thanks for playing!")
        self.root.destroy()
    ...
    def log_attempt(self, question, user_answer, correct_answer, is_correct):
        ...
        log_entry = (f"[{timestamp}] Q: {clean_question} | "
                    ...
                    f"HeroHP {self.hero_hp}/{self.hero_max_hp} | "
                    f"MonsterHP {self.monster_hp}/{self.monster_max_hp}\n")
        try:
...
5. NEW: Battle monsters by answering correctly!

💥 Battle Rules:
• Correct answer = You damage the monster
• Wrong answer = The monster damages you
• When the monster's HP hits 0 → next level (stronger monster, small heal)
    ...
    def exit_game(self):
        if self.total_questions > 0:
            ...
            stats = (f"📊 Final Stats:\n"
                    ...
                    f"   Highest Dungeon Level: {self.level}\n"
                    f"   Achievements: {len(self.achievements)}\n\n")
